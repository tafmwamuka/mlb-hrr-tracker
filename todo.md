...
- [x] Rewrite Stats tab PerformanceDashboard.tsx to use trpc.tracking.getCumulativeRecord and trpc.tracking.getResultsForDate exclusively. Removed all calls to trpc.history.getSevenDayStats, trpc.history.getPerformanceSummary, trpc.results.getHitRateStats, trpc.results.getYesterdayResults. TypeScript: 0 errors. Tests: 229/231.
- [x] Fix sub-tab bar (Intel/Today/Grades/Leaders/Edge Log) in PitchersTab.tsx: added sticky+z-20 wrapper so the bar stays above the lean/parlay collapsible section; bumped inactive tab contrast from text-white/30 to text-white/60 with font-semibold so labels are clearly visible
- [x] Fix Eddy Yean Book:+0/Edge:0.0% (same class as Byron Buxton bug): in discipline.ts serializePick, null-guard bookOdds and edge when hasMarketData=false; updated PitcherPickCard in both PitcherEdgePicksClean.tsx and PitcherEdgePicks.tsx to render "N/A" instead of +0/0.0% when odds are unavailable
